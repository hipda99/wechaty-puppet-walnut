...
class Walnut extends EventEmitter {
  ...
  constructor (
    ...
    super()
    log.verbose('Walnut', 'constructor(%s)', JSON.stringify(options))
  ...
  verify (args: VerifyArgs): boolean {
    log.verbose('Walnut', 'verify(%s)', JSON.stringify(args))
  ...
  async start () {
    ...
    if (!succeed) {
      log.error('Walnut', 'start() updateAccessToken() failed.')
    }
  ...
  protected async updateAccessToken (): Promise<boolean> {
    log.verbose('Walnut', 'updateAccessToken()')
    ...
    log.verbose('Walnut', 'updateAccessToken() %s', JSON.stringify(ret.body))
    ...
    if (ret.body.errcode && ret.body.errcode > 0) {
      ...
      log.warn('Walnut', `updateAccessToken() ${ret.body.errcode}: ${ret.body.errmsg}`)
      ...
      if (this.accessTokenPayload) {
        ...
        if (expireTimestamp > Date.now()) {
          // expired.
          log.warn('Walnut', 'updateAccessToken() token expired!')
    ...
    log.verbose('Walnut', 'updateAccessToken() synced. New token will expiredIn %s seconds',
  ...
  protected async startSyncingAccessToken (): Promise<StopperFn> {
    log.verbose('Walnut', 'startSyncingAccessToken()')
    ...
    const update = () => this.updateAccessToken()
      ...
      .catch(e => log.error('Walnut', 'startSyncingAccessToken() update() rejection: %s', e))
  ...
  async sendCustomMessage (args: {
    ...
    log.verbose('Walnut', 'sendCustomMessage(%s)', JSON.stringify(args))
    ...
    if (ret.body.errcode) {
      throw new Error(`Walnut sendCustomMessage() can send message <${JSON.stringify(args)}>`)
    }
  ...
  async sendFile (args: {
    ...
    log.verbose('Walnut', 'sendFile(%s)', JSON.stringify(args))
    ...
    if (messageResponse.body.errcode) {
      log.error('Walnut', 'SendFile() can not send file to wechat user .<%s>', messageResponse.body.errmsg)
      throw new Error(`Walnut', 'SendFile() can not send file to wechat user .<${messageResponse.body.errmsg}>'`)
    }
  ...
  async getContactList (): Promise<string[]> {
    log.verbose('Walnut', 'getContactList')
    ...
    while (true) {
      ...
      if (req.body.errcode) {
        log.error(`Walnut', 'getContactList() ${req.body.errmsg}`)
        return openIdList
      }
  ...
  async getContactPayload (openId: string): Promise<void | OAContactPayload> {
    log.verbose('Walnut', 'getContactPayload(%s)', openId)
    ...
    if (res.body.errcode) {
      log.error(`Walnut', 'getContactPayload() ${res.body.errmsg}`)
      return
    }
  ...
  async updateContactRemark (openId: string, remark: string): Promise<void> {
    log.verbose('Walnut', 'setContactRemark(%s)', JSON.stringify({ openId, remark }))

    const res = await this.simpleUnirest.post<ErrorPayload>(`user/info/updateremark?access_token=${this.accessToken}`)

    if (res.body.errcode) {
      log.error('Walnut', 'setContactRemark() can update contact remark (%s)', res.body.errmsg)
    }
  }

  async createTag (name: string): Promise<void | string> {
    log.verbose('Walnut', 'createTag(%s)', name)
    ...
    if (res.body.errcode) {
      log.error('Walnut', 'createTag(%s) error code : %s', name, res.body.errcode)
    } else {
      return name
    }
  }
  ...
  async getTagList (): Promise<OATagPayload[]> {
    ...
    if (res.body.errcode) {
      log.error('Walnut', 'getTagList() error code : %s', res.body.errcode)
      return []
    }

    if (!res.body.tags || res.body.tags.length === 0) {
      log.warn('Walnut', 'getTagList() get empty tag list')
      return []
    }
  ...
  private async getTagIdByName (tagName: string): Promise<number| null> {
    log.verbose('Walnut', 'deleteTag(%s)', tagName)
  ...
  async deleteTag (tagName: string): Promise<void> {
    ...
    if (res.body.errcode) {
      log.error('Walnut', 'deleteTag() error code : %s', res.body.errcode)
    }
  }

  async addTagToMembers (tagName: string, openIdList: string[]): Promise<void> {
    log.verbose('Walnut', 'addTagToMembers(%s)', JSON.stringify({ tagName, openIdList }))
    ...
    if (res.body.errcode) {
      log.error('Walnut', 'addTagToMembers() error code : %s', res.body.errcode)
    }
  }

  async removeTagFromMembers (tagName: string, openIdList: string[]): Promise<void> {
    log.verbose('Walnut', 'removeTagFromMembers(%s)', JSON.stringify({ tagName, openIdList }))
    ...
    if (res.body.errcode) {
      log.error('Walnut', 'removeTagFromMembers() error code : %s', res.body.errcode)
    }
  }

  async getMemberTags (openid: string): Promise<string[]> {
    log.verbose('Walnut', 'getMemberTags(%s)', openid)
    ...
    if (res.body.errcode) {
      throw new Error(`Walnut deleteTag() error code : ${res.body.errcode}`)
    }
  ...
  async setMemberRemark (openid: string, remark: string): Promise<void> {
    log.verbose('Walnut', 'setMemberRemark(%s)', openid)
  ...
  async sendBatchTextMessageByTagId (tagId: number, msg: string): Promise<void> {
    log.verbose('Walnut', 'sendBatchTextMessageByTagId(%s)', JSON.stringify({ tagId, msg }))
  ...
  async sendBatchTextMessageByOpenidList (openidList: string[], msg: string): Promise<void> {
    log.verbose('Walnut', 'sendBatchTextMessageByOpenidList(%s)', JSON.stringify({ openidList, msg }))
